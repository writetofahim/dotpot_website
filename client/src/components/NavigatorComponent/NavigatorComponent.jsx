...
const NavigatorComponent = ({ navigationData }) => {
  ...
  return (
      ...
      <div className="w-max flex text-xl backdrop-blur-lg">
        <Link
          to="/"
          className="ml-2 text-primary-500 hover:text-secondary-500 md:text-base text-xs"
        >
          Home
        </Link>
        {navigationData?.map((item, index) => (
          <div key={index} className="cursor-pointer ml-1 md:text-base text-xs text-primary-500">
            &gt;&gt;
            <Link
              to={item.link}
              key={index}
