...
function SampleNextArrow(props) {
  ...
  return (
    <GrNext
      className="md:block text-4xl md:text-6xl absolute right-0 top-1/2 cursor-pointer opacity-30 hover:opacity-100 hover:bg-secondary-500 transition-all -translate-y-[50%]  p-2 rounded-full translate-x-[80%] "
      onClick={onClick}
    />
  );
}
...
function SamplePrevArrow(props) {
  ...
  return (
    <GrPrevious
      className="md:block z-10 text-4xl md:text-6xl absolute left-0 top-1/2 cursor-pointer opacity-30 hover:opacity-100 hover:bg-secondary-500 transition-all -translate-y-[50%]  p-2 rounded-full -translate-x-[80%] "
      onClick={onClick}
    />
  );
}
  ...
  return (
    <Link to={props.link}>
      <div className="z-2 bg-white md:w-[1/8] mx-2 h-[200px] p-3 rounded-xl shadow flex flex-col items-center justify-center hover:border hover:border-primary-500 transition-all">
        ...
        <p className="mt-5 text-center hover:text-secondary-500 text-xl font-bold">
          {props.title}
        </p>
      </div>
    </Link>
