            ...
            data.map((item, index) => {
              return (
                <Link to={item.url} key={index} target="_blank">
                  <div
                    key={index}
                    className="md:w-[100px] md:h-[100px] p-2 rounded-full md:rounded-lg shadow flex items-center justify-center hover:scale-110 hover:shadow-xl transition-all glassmorphism"
                  >
                    <img
                      src={item.icon}
                      alt=""
