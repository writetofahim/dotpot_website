...
export default function AllWorks() {
  ...
  const fetchData = async (page) => {
    try {
      const response = await axios.get(`/work?page=${page}`);
    ...
    <div className='lg:p-5 p-3'>
      ...
      <div className="relative overflow-x-auto">
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {data?.map((work) => (
              <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={work._id}>
                <WorkTableRow work={work} setData={setData} data={data} />
              </tr>
            ))}
          </tbody>
        </table>
